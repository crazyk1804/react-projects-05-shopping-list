...
const ItemsContextProvider = ({children}) => {
	...
	const getItemsRequest = async (id) => {
		const result = await fetchData(
			`https://my-json-server.typicode.com/crazyk1804/react-projects-05-shopping-list/items/${id}/items`
		);
	...
	const addItemRequest = async content => {
		const result = await postData(
			`https://my-json-server.typicode.com/crazyk1804/react-projects-05-shopping-list/items`,
			content
		);
